...
운영자는 `operator01`, `operator02`, `operator03` 세 아이디를 사용합니다. 실제 Auth 계정은 서버 전용 일회성 스크립트로 생성하며 이메일 확인은 요구하지 않습니다. 다음 환경 변수를 터미널 세션이나 안전한 비밀 저장소에서 설정합니다.
...
OPERATOR03_PASSWORD=<12자 이상의 고유 비밀번호>
```
...
스크립트는 멱등하게 동작하여 재실행 시 기존 운영자 계정만 갱신합니다. 기존 관리자이거나 다른 역할의 계정과 주소가 충돌하면 즉시 중단하며 관리자 계정은 수정하지 않습니다.
...
| 운영자 (`operator`) | `operator01~03`와 비밀번호 | 전체 상담함·회원 현황 조회, 상품 단건/일괄 등록·수정·삭제 |
...
- 기존 관리자 로그인이 유지되고 `operator01~03`이 운영자 권한만 갖는지
