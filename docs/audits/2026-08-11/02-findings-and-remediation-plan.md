...
`747043f`의 3분 역할 카나리로 실제 소유자 UUID를 저장 역할 변경 없이 다미네 운영자와 직원 principal에 각각 연결했다. 운영자 rehearsal은 owner bypass가 false였고 다미네 상품 관리·공개·주문 준비만 허용됐으며, 배송센터 수령·배송 생성과 나인티 나인 매장 상품 관리는 차단됐다. 직원 rehearsal은 다미네 상품 관리만 허용되고 공개·직원 관리·보고서·타 매장 상품 관리가 차단됐다. 직원의 운영자 센터 선택과 물류 권한 호출은 `42501`로 차단됐다. 모든 lease는 명시 종료됐고 저장 역할은 owner로 복원됐으며 감사 8행이 보존됐다.
