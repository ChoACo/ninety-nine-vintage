...
export default async function MobileAccountSectionPage({ params }: { params: Promise<{ section: string }> }) {
  ...
  return <MemberAccountBoundary basePath="/m" returnTo={returnTo}><div data-account-task={section}><div className="mb-5 border-b border-line pb-4"><p className="text-xs font-bold text-muted">MY · {sectionLabels[section as AccountSection]}</p><h1 className="mt-2 text-2xl font-black tracking-[-.06em]">{sectionLabels[section as AccountSection]}</h1></div>{["storage", "shipping-request", "shipping", "addresses"].includes(section) && <AccountShippingNav basePath="/m" current={section} />}<AccountDashboard basePath="/m" view={view} /></div></MemberAccountBoundary>;
}
