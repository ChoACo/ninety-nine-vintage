...
import { DualScrollLayout } from "@/components/layout/DualScrollLayout";
...
export default async function ShopPage({searchParams}:{searchParams:Promise<{q?:string|string[];category?:string|string[]}>}){const params=await searchParams;const query=typeof params.q==="string"?params.q:"";const category=typeof params.category==="string"?params.category:"";const products=await fetchPublishedProducts({limit:100,saleType:"fixed",search:query});return <DualScrollLayout sidebar={<AuctionFilterSidebar presentation="sidebar" saleType="fixed" surface="desktop"/>} sidebarLabel="아카이브 숍 상세 필터"><div className="space-y-8"><div className="md:hidden"><AuctionFilterSidebar saleType="fixed" surface="desktop"/></div><ArchiveShopHeader category={category}/><Suspense fallback={<ShopCatalogSkeleton/>}><AuctionFeedGrid initialProducts={toPayload(products)} saleType="fixed" surface="desktop" title="엄선된 1점 한정 아카이브"/></Suspense></div></DualScrollLayout>}
