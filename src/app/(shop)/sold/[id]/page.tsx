...
export default async function SoldDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <article className="space-y-8 md:space-y-10"><script dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd).replace(/</g, "\\u003c") }} type="application/ld+json" /><header className="border-b border-ink pb-7"><p className="eyebrow text-muted">판매 기록 / 판매 완료</p><p className="mt-4 text-xs font-bold text-muted">{product.brand} · {product.category}</p><h1 className="mt-2 break-keep text-3xl font-black tracking-[-.08em] md:text-4xl">{product.title}</h1><p className="mt-4 max-w-3xl text-sm leading-6 text-muted">{product.description}</p></header><div className="grid gap-8 lg:grid-cols-2 lg:gap-10"><div className="space-y-3">{product.image_urls.map((image, index) => <div className="relative aspect-[4/5] overflow-hidden bg-surface" key={image}><CatalogImage alt={`${product.title} ${index + 1}`} className="h-full w-full object-cover" src={image} /></div>)}</div><aside className="self-start border-t-2 border-ink lg:sticky lg:top-28"><dl className="divide-y divide-line text-sm"><div className="flex justify-between gap-4 py-5"><dt className="text-muted">낙찰가</dt><dd className="font-mono text-xl font-bold">{product.winning_amount.toLocaleString("ko-KR")}원</dd></div><div className="flex justify-between gap-4 py-4"><dt className="text-muted">판매일</dt><dd className="text-right">{new Intl.DateTimeFormat("ko-KR", { dateStyle: "long" }).format(new Date(product.sold_at))}</dd></div><div className="flex justify-between gap-4 py-4"><dt className="text-muted">상태 등급</dt><dd className="flex items-center gap-1 font-bold"><BadgeCheck size={14} /> {product.condition_grade}</dd></div>{product.size_label && <div className="flex justify-between gap-4 py-4"><dt className="text-muted">표기 사이즈</dt><dd className="text-right">{product.size_label}</dd></div>}</dl>{measurements.length > 0 && <section className="mt-8 border-y border-line"><h2 className="flex items-center gap-2 border-b border-line py-4 text-xs font-bold"><Ruler size={14} /> 실측 사이즈</h2>{measurements.map((measurement) => <div className="flex justify-between border-b border-line py-3 text-xs last:border-b-0" key={measurement.label}><span className="text-muted">{measurement.label}</span><span className="font-mono">{measurement.value}cm</span></div>)}</section>}<section className="mt-8 bg-surface p-5"><h2 className="text-xs font-bold">검수 기록</h2><ul className="mt-3 space-y-2 text-xs leading-5 text-muted">{product.inspection_notes.length > 0 ? product.inspection_notes.map((note) => <li key={note}>{note}</li>) : <li>특이사항 없음</li>}</ul></section></aside></div></article>;
}
