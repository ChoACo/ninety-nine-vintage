...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!store) return {};
  const title = `${store.name} 즉시구매 | NINETY-NINE VINTAGE`;
...
export default async function StoreBuyNowPage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ date?: string }> }) {
  ...
  return <StoreMallCatalogPage basePath="" dates={dates} products={fixed} selectedDate={selectedDate} slug={slug} store={store} surface="desktop" tab="buy" />;
}