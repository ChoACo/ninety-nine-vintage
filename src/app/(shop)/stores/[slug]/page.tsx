...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!store) return {};
  const title = `${store.name} 센터몰 | NINETY-NINE VINTAGE`;
...
export default async function StorePage({ params, searchParams }: { params: Promise<{ slug: string }>; searchParams: Promise<{ date?: string }> }) {
  ...
  return <StoreMallExperience auctions={auctions} dates={dates} fixed={fixed} selectedDate={selectedDate} slug={slug} sold={[...soldFixed, ...soldAuctions]} store={store} surface="desktop" />;
}
