...
export async function GET(request: Request) {
 try {
  ...
  if (storeId) {
    ...
    vault = vault.eq("origin_store_id", storeId);
  }
  const [storeResult, auditResult, settlementResult, auctionResult, vaultResult, shipmentResult, sessionResult] = await Promise.all([
    ...
    auth.admin.from("security_activity_logs").select("id", { count: "exact", head: true }), settlements.limit(10_000), auctions.limit(10_000), vault.limit(10_000), auth.user.rpc("get_owner_shipped_inventory_flow", { p_from: currentFrom.toISOString(), p_store_id: storeId }),
  ...
  const vaultFlow = Array.from({ length: 14 }, (_, index) => { const key = new Date(currentFrom.getTime() + index * DAY).toISOString().slice(0, 10); return { date: key.slice(5).replace("-", "/"), stored: vaultRows.filter((row) => row.storage_started_at?.slice(0, 10) === key).length, shipped: shipmentRows.filter((row) => row.shipped_at?.slice(0, 10) === key).length }; });
