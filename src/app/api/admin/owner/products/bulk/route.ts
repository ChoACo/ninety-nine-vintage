...
function normalize(body: Record<string, unknown>, ownerId: string): ProductInsert | null {
  ...
  if (!text(body.title) || !text(body.description) || !text(body.storeId) || images.length === 0 || !Number.isSafeInteger(startingPrice) || startingPrice <= 0 || (fixedPrice !== null && (!Number.isSafeInteger(fixedPrice) || fixedPrice <= 0))) return null;
  const price = saleType === "fixed" ? fixedPrice as number : startingPrice;
  return { title: text(body.title), description: text(body.description), category: text(body.category, "구제 의류"), store_id: text(body.storeId), sale_type: saleType, fixed_price: fixedPrice, starting_price: price, current_price: price, bid_increment: 1000, image_urls: images, thumbnail_urls: images, publish_at: text(body.publishAt, new Date().toISOString()), closes_at: text(body.closesAt, new Date(Date.now() + 86400000).toISOString()), status: "pending", created_by: ownerId, updated_by: ownerId, size_label: text(body.sizeLabel), condition_grade: ["S", "A+", "A", "B"].includes(text(body.conditionGrade)) ? text(body.conditionGrade) : "A", storage_class: text(body.storageClass) === "large" ? "large" : "small", measurements: {}, inspection_notes: [] };
}
