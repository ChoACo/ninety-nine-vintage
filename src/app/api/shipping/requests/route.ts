...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { productIds?: unknown; addressId?: string; applyShippingCredit?: boolean } | null;
  ...
  if (!body.applyShippingCredit) {
    ...
    if (paymentError) return commerceJson({ error: "배송비 결제 안내를 만들지 못했습니다." }, 503);
  }
