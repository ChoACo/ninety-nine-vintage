...
export interface CollapsibleSectionProps {
  ...
  className?: string;
  /**
   * When provided, the section becomes a workspace panel controlled by its
   * parent. Uncontrolled accordion behaviour remains unchanged when omitted.
   */
  active?: boolean;
  /** Whether a controlled panel has already been visited and should stay mounted. */
  visited?: boolean;
}
...
export function CollapsibleSection({
  ...
  const isExpanded = isControlled ? active : isOpen;
  ...
  const heading = (
    <>
      <span className="min-w-0">
        <span className="block font-mono text-[10px] font-black uppercase tabular-nums tracking-[0.18em] text-[var(--accent-text)]">
          {eyebrow}
        </span>
  ...
  return (
    <section
      id={id}
      role={isControlled ? "tabpanel" : undefined}
      aria-labelledby={isControlled && id ? `${id}-tab` : undefined}
      aria-hidden={isControlled && !isExpanded}
      hidden={isControlled && !isExpanded}
      className={`scroll-mt-24 overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-raised)] shadow-[var(--panel-shadow)] transition-all duration-200 ease-out hover:border-[var(--border-strong)] ${
      ...
      <div className="relative flex items-start gap-3 px-4 py-3.5 sm:px-5 sm:py-4">
        ...
        {isControlled ? (
          ...
          <button
            ...
            className="group flex min-w-0 flex-1 items-center justify-between gap-4 rounded-lg text-left outline-none transition-all duration-200 active:scale-[0.995] focus-visible:ring-2 focus-visible:ring-[var(--accent)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface-raised)]"
          >
            {heading}
          </button>
        )}
