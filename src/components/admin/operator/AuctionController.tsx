...
export function AuctionController({ productId, title, onChanged }: { productId: string; title: string; onChanged: () => void }) {
  ...
  return <>
    ...
    <PremiumDialog ariaLabel="실시간 경매 제어" labelledBy="auction-controller-title" onClose={() => setOpen(false)} open={open} panelClassName="max-w-4xl">
      ...
      <div className="mt-5 divide-y divide-line md:hidden">{bids.map((bid, index) => <article className={`rounded-xl px-3 py-4 ${index === 0 ? "bg-emerald-500/10" : ""}`} key={bid.id}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-black">#{index + 1} · {bid.bidder_display_name}</p><time className="mt-1 block text-[10px] text-muted">{new Date(bid.created_at).toLocaleTimeString("ko-KR")}</time></div><strong className="shrink-0 font-mono text-sm">{bid.amount.toLocaleString("ko-KR")}원</strong></div><div className="mt-3 grid grid-cols-2 gap-2"><button className="inline-flex min-h-11 items-center justify-center gap-1 border border-line px-3 text-xs font-bold" onClick={() => choose("cancel_bid", bid)} type="button"><RotateCcw size={13} />입찰 취소</button><button className="inline-flex min-h-11 items-center justify-center gap-1 border border-red-300 px-3 text-xs font-bold text-red-700" onClick={() => choose("block_bidder", bid)} type="button"><Ban size={13} />입찰 차단</button></div></article>)}{bids.length === 0 && <p className="py-12 text-center text-xs text-muted">아직 입찰 기록이 없습니다.</p>}</div>
