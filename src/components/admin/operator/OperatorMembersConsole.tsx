...
export function OperatorMembersConsole() {
  ...
  const [staffRole, setStaffRole] = useState<string>("operator");
  ...
  const load = useCallback(async (accessToken: string | null) => {
    ...
    const response = await fetch("/api/admin/operator/members?limit=500", { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
    const payload = await response.json() as { members?: Member[]; roleCode?: string; error?: string };
    if (!response.ok) throw new Error(payload.error ?? "회원 목록을 불러오지 못했습니다.");
    setMembers(payload.members ?? []);
    setStaffRole(payload.roleCode ?? "operator");
  }, []);
  ...
  const patchMember = async (body: Record<string, unknown>, message: string) => {
    ...
    try {
      const response = await fetch("/api/admin/operator/members", { method: "PATCH", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify(body) });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error ?? message);
      setNotice(message); await load(token);
      if (selected) setSelected((current) => current ? { ...current } : current);
