  ...
  return <div className="space-y-8">
    ...
    <div className="divide-y divide-line border-y border-line">{orders.map(({ transfer, lines }) => {
      ...
      return <article className="py-6" key={transfer.id}>
        ...
        {transfer.remainingAmount > 0 && <div className="mt-4 grid grid-cols-[140px_150px_1fr_auto] gap-2"><TextInput aria-label={`${transfer.order_id} 입금액`} className="h-10" inputMode="numeric" onChange={(event) => updateForm(transfer.id, { amount: event.target.value })} placeholder={`최대 ${formatWon(transfer.remainingAmount)}`} value={form.amount} /><TextInput aria-label={`${transfer.order_id} 입금자명`} className="h-10" onChange={(event) => updateForm(transfer.id, { depositorName: event.target.value })} placeholder="입금자명" value={form.depositorName} /><TextInput aria-label={`${transfer.order_id} 메모`} className="h-10" onChange={(event) => updateForm(transfer.id, { memo: event.target.value })} placeholder="메모 (선택)" value={form.memo} /><Button className="h-10" disabled={busy === transfer.id || !form.amount || !form.depositorName} variant="outline" onClick={() => void mutateLedger(transfer, { action: "record", kind: "commerce", amount: Number(form.amount.replaceAll(",", "")), depositorName: form.depositorName, memo: form.memo })} type="button">입금 기록</Button></div>}
