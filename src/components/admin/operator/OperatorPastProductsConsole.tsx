...
export function OperatorPastProductsConsole() {
  ...
  const load = useCallback(async (accessToken: string | null) => {
    ...
    setLoading(true);
    try {
      const response = await fetch("/api/admin/operator/products/past", { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
  ...
  return <div className="space-y-8">
    <SectionHeading action={<Button className="flex items-center gap-2" disabled={loading} onClick={() => void load(token)} type="button"><RefreshCw size={13} /> 새로고침</Button>} description="7일이 지난 미낙찰 경매는 최대 3일 동안 이곳에서 재등록하거나 삭제할 수 있습니다." eyebrow="OPERATOR / PAST AUCTIONS" title="지난 상품" variant="page" />
    ...
    <div className="flex items-center justify-between border-y border-line py-4 text-xs">
      ...
      <div className="flex gap-2"><Button className="flex items-center gap-2 text-xs" disabled={busy || selected.length === 0} size="compact" variant="outline" onClick={() => void act("relist")} type="button"><RotateCcw size={13} /> 선택 재등록</Button><Button className="flex items-center gap-2 text-xs" disabled={busy || selected.length === 0} size="compact" variant="danger" onClick={() => void act("delete")} type="button"><Trash2 size={13} /> 선택 삭제</Button></div>
    </div>
