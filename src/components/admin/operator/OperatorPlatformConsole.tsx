...
export function OperatorPlatformConsole() {
  ...
  async function load(){const response=await fetch('/api/admin/operator/platform',{cache:'no-store'});const payload=await response.json();if(response.ok)setStores(payload.management?.stores??[]);else setMessage(payload.error??'조회 실패');}
  useEffect(()=>{
    let active=true;
    void fetch('/api/admin/operator/platform',{cache:'no-store'})
  ...
  async function action(body:Record<string,unknown>){setMessage('처리 중…');const response=await fetch('/api/admin/operator/platform',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify(body)});const payload=await response.json();setMessage(response.ok?'저장했습니다.':payload.error??'처리 실패');if(response.ok)void load();}
