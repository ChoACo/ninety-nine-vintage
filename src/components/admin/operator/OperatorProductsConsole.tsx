...
export function OperatorProductsConsole() {
  ...
  const remove = async (product: Product) => {
    ...
    setBusy(true); setNotice("");
    try {
      const response = await fetch(`/api/admin/operator/products/${product.id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
      const payload = await response.json() as { error?: string };
  ...
  return <div className="space-y-8">
    ...
    <form className="grid grid-cols-2 gap-3 border border-ink bg-surface p-6" onSubmit={submit}>
      ...
      <TextInput aria-label="사이즈" onChange={(event) => update("sizeLabel", event.target.value)} placeholder="사이즈·실측 요약" value={form.sizeLabel} />
      <div className="flex gap-2"><SelectInput aria-label="판매 방식" className="flex-1" onChange={(event) => update("saleType", event.target.value)} value={form.saleType}><option value="fixed">즉시구매</option><option value="auction">경매</option></SelectInput><TextInput aria-label="가격" className="w-40" min="1" onChange={(event) => update("price", event.target.value)} placeholder="가격" required type="number" value={form.price} /></div>
      <div className="flex gap-2"><SelectInput aria-label="컨디션" className="flex-1" onChange={(event) => update("conditionGrade", event.target.value)} value={form.conditionGrade}><option value="S">S</option><option value="A+">A+</option><option value="A">A</option><option value="B">B</option></SelectInput><SelectInput aria-label="보관 등급" className="flex-1" onChange={(event) => update("storageClass", event.target.value)} value={form.storageClass}><option value="small">소형 · 14일</option><option value="large">대형 · 7일</option></SelectInput></div>
      <TextArea aria-label="이미지 URL" className="col-span-2 min-h-20" onChange={(event) => update("imageUrls", event.target.value)} placeholder="이미지 URL을 줄바꿈 또는 쉼표로 입력" required value={form.imageUrls} />
