...
export function OperatorProductsConsole({
  ...
  return <div className="space-y-8">
    <SectionHeading
      description={view === "active" ? "현재 공개 중인 상품과 결제 진행 중·낙찰 대기로 선점된 상품을 판매 방식별로 나누어 관리합니다." : "신규 상품을 등록하고 업로드 예정 상품과 등록 대기 상품을 관리합니다."}
          ...
          <>
            ...
            {form.saleType === "auction" ? (
              <div className="flex flex-col gap-1.5 sm:col-span-2">
                <label className="text-[10px] font-bold text-muted"><span className="mb-2 block">입찰 단위</span><TextInput aria-label="입찰 단위" min="1" onChange={(event) => update("bidIncrement", event.target.value)} placeholder="입찰 단위 (기본 1,000원)" type="number" value={form.bidIncrement} /></label>
                <p className="text-[11px] leading-5 text-muted">호가 단위는 상품별로 설정하는 값이며 센터 공통 설정은 없습니다. 시작가 {Number(form.price || 0).toLocaleString("ko-KR")}원 기준 첫 입찰은 시작가부터 시작하고, 이후 입찰은 현재가에서 호가 단위만큼 올라갑니다. 등록 대기 상태에서는 수정할 수 있고 공개 후에는 변경할 수 없습니다.</p>
              </div>
            ) : (
