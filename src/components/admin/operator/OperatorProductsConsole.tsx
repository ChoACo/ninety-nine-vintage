...
export function OperatorProductsConsole() {
  ...
  return <div className="space-y-8">
    ...
    <section className="border border-line bg-surface p-4 sm:p-6"><div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:items-center"><div className="flex items-start gap-2"><Upload className="mt-0.5 shrink-0" size={15} /><div><p className="text-sm font-bold">상품 일괄 등록</p><p className="mt-1 break-words text-[11px] text-muted">CSV 필수 헤더: title, brand, description, storeId, saleType, startingPrice, imageUrls. 이미지 주소는 | 로 구분합니다.</p></div></div><button className="flex shrink-0 items-center gap-2 text-xs underline" onClick={downloadTemplate} type="button"><Download size={13} /> 양식 저장</button></div><textarea aria-label="일괄 등록 CSV" className="mt-4 min-h-32 w-full border border-line bg-paper p-3 font-mono text-[11px]" disabled={!permissions.canCreate} onChange={(event) => setBulkText(event.target.value)} value={bulkText} /><button className="mt-3 bg-ink px-5 py-3 text-xs font-bold text-paper disabled:opacity-40" disabled={busy || !token || !permissions.canCreate} onClick={() => void bulk()} type="button">CSV 일괄 등록 실행</button></section>
