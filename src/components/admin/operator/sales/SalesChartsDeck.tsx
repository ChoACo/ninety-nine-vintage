...
export function SalesChartsDeck({ entries }: { entries: SalesEntry[] }) {
  ...
  return <section aria-label="매출 차트" className="grid gap-4 xl:grid-cols-[1.6fr_1fr]">
    <article className="rounded-2xl border border-zinc-800 bg-zinc-900 p-5">
      ...
      <div className="mt-5 h-80"><ResponsiveContainer height="100%" width="100%"><AreaChart data={daily}>
        ...
        <Tooltip contentStyle={tooltipStyle} formatter={(value) => `₩${Number(value).toLocaleString("ko-KR")}`} /><Legend wrapperStyle={{ fontSize: 12 }} />
