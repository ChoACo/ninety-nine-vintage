...
export function OwnerBusinessAnalytics({ revenue, auction, vaultFlow }: { revenue: RevenuePoint[]; auction: { sold: number; unsold: number }; vaultFlow: VaultPoint[] }) {
  ...
  return <section className="space-y-4" aria-labelledby="owner-analytics-title">
    ...
    <div className="grid gap-4 xl:grid-cols-3">
      ...
      <article className="min-w-0 border border-line bg-surface p-4"><h3 className="text-xs font-black">보관함 입고 · 묶음 출고</h3><div className="mt-4 h-64" aria-label="보관함 입고와 묶음 출고 추이"><ResponsiveContainer height="100%" width="100%"><BarChart data={vaultFlow}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="date" fontSize={10} /><YAxis allowDecimals={false} fontSize={10} width={28} /><Tooltip /><Bar dataKey="stored" fill="#d97706" name="입고" /><Bar dataKey="shipped" fill="#059669" name="출고" /></BarChart></ResponsiveContainer></div></article>
    </div>
  </section>;
}
