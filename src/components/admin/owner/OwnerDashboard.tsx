  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 gap-px border border-line bg-line sm:grid-cols-3">
        <div className="bg-paper p-6"><Store size={17} /><p className="mt-8 text-xs text-muted">운영 중인 숍</p><p className="mt-2 font-mono text-3xl font-bold">{stores.length}</p></div>
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {stores.length === 0 && <div className="col-span-full border border-dashed border-line py-12 text-center text-sm text-muted">등록된 숍이 없습니다.</div>}
        </div>
      </section>
    </div>
  );
