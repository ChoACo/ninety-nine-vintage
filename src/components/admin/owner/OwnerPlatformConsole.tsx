...
export function OwnerPlatformConsole() {
  ...
  const revealPayoutAccount=async(store:StoreItem)=>{
    ...
    if(account?.accountNumber) setRevealedPayoutAccount({
      storeId: store.id,
  ...
  return <div className="space-y-6">
    <SectionHeading eyebrow="소유자 / 다중 센터" title="출고 그룹·회원제·정산 관리" description="센터 격리는 유지하면서 묶인 센터의 출고 협업과 구매자 배송비 청구 단위를 관리합니다." variant="page" action={<div className="flex flex-wrap gap-2"><ExportDataButton/><Button disabled={!token || busy} onClick={() => token && void load(token)} type="button"><RefreshCw size={14} /> 새로고침</Button></div>} />
