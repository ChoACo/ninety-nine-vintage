...
export function AccountDashboard() {
  ...
  return <div className="space-y-14"><div className="flex flex-col justify-between gap-5 border-b border-ink pb-8 md:flex-row md:items-end"><div><p className="eyebrow text-muted">MY ACCOUNT / LIVE DATA</p><h1 className="mt-3 text-4xl font-black tracking-[-0.08em]">안녕하세요, {userName}.</h1><p className="mt-3 text-sm text-muted">나의 경매와 보관, 배송을 한 곳에서 관리하세요.</p></div>{token ? <span className="flex items-center gap-2 border border-emerald-200 bg-emerald-50 px-4 py-3 text-xs font-bold text-emerald-800"><UserRound size={15} /> 로그인 상태</span> : <a className="inline-flex items-center gap-2 border border-line px-4 py-3 text-xs font-bold" href="/api/auth/kakao/start?returnTo=%2Faccount"><LogIn size={15} /> 카카오로 로그인하기</a>}</div>
