...
function AccountDashboardForSession({
  ...
  const [paymentServerTime, setPaymentServerTime] = useState<string | null>(
    null,
  );
  ...
  const [pendingDeleteAddressId, setPendingDeleteAddressId] = useState<
    string | null
  >(null);
  const [legacyAuctionWins, setLegacyAuctionWins] = useState<
    LegacyAuctionWin[]
  >([]);
  const [deadlineEnforcementExempt, setDeadlineEnforcementExempt] =
    useState(false);
  const [rememberedDepositorName, setRememberedDepositorName] = useState<
    string | null
  >(null);
  const [paymentGroups, setPaymentGroups] = useState<
    AuctionPaymentCenterGroup[]
  >([]);
  const [centerShippingTokens, setCenterShippingTokens] = useState<
    CenterShippingToken[]
  >([]);
  ...
  const [purchaseConfirmationShipment, setPurchaseConfirmationShipment] =
    useState<InventoryShipment | null>(null);
  const [purchaseConfirmationBusy, setPurchaseConfirmationBusy] =
    useState(false);
  const [showAllStorage, setShowAllStorage] = useState(false);
  const [refundMessage, setRefundMessage] = useState("");
  const [refundBusyId, setRefundBusyId] = useState<string | null>(null);
  const [refundDrafts, setRefundDrafts] = useState<
    Record<string, RefundAccountDraft>
  >({});
    ...
    const load = async () => {
      ...
      try {
        const [
          ...
          fetch("/api/account/legacy-eligible-orders", {
            headers,
            cache: "no-store",
          }),
        ]);
        ...
        const requiresMemberAccess = problemPayloads.some(
          (payload) =>
            payload?.code === "member_required" ||
            payload?.error === "member_required",
        );
        ...
        if (!cancelled) {
          ...
          if (requiresMemberAccess) {
            setNotice(
              "현재 로그인한 계정은 일반 회원 계정이 아닙니다. 회원 계정으로 다시 로그인해 주세요.",
            );
          }
          setDataStatus("ready");
        }
  ...
  const selectedLegacyOrder = useMemo(
    () =>
      legacyEligibleOrders.find(
        (order) => order.sourceId === selectedOrderId,
      ) ?? null,
  ...
  const showOverview =
    view === "full" || view === "simple" || view === "overview";
  const showPayments =
  ...
  const requestEligibleItems = useMemo(
    () =>
      v2Storage.filter(
        (item) => item.requestEligible && !item.activeShipmentId,
      ),
    [v2Storage],
  );
  ...
  const allRequestEligibleSelected =
    visibleRequestEligibleItems.length > 0 &&
    visibleRequestEligibleItems.every((item) =>
  ...
  const hasHiddenStorage =
    ...
    visibleSettledLegacyAuctionWins.length < settledLegacyAuctionWins.length;
  ...
  const deleteAddress = async (address: Address) => {
    ...
    try {
      ...
      if (selectedAddressId === address.id) {
        setSelectedAddressId(
          next.find((item) => item.is_default)?.id ?? next[0]?.id ?? "",
        );
      }
  ...
  const requestShipping = async () => {
    ...
    if (
      ...
      setShippingMessage("배송 신청 상품과 배송지를 선택해 주세요.");
      return;
    }
    ...
    if (useV2) {
      ...
      setLegacyEligibleOrders((current) =>
        current.filter(
          (order) => order.sourceId !== selectedLegacyOrder?.sourceId,
        ),
      );
      setSelectedOrderId("");
    }
    ...
    setShipments((current) => [
      ...
      ...current.filter(
        (currentShipment) => currentShipment.id !== shipment.shipment_id,
      ),
    ]);
  ...
  const submitRefundAccount = async (refund: ManualRefund) => {
    ...
    const idempotencyKey =
      refundAccountKeys.current.get(scope) ?? crypto.randomUUID();
    refundAccountKeys.current.set(scope, idempotencyKey);
    setRefundBusyId(subjectKey);
    setRefundMessage("");
    try {
      const response = await fetch(
        `/api/account/refunds/${refund.id}/account`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (
        !response.ok ||
        !payload.refund ||
        payload.refund.id !== refund.id ||
        payload.refund.refundKind !== refund.refundKind
      ) {
      ...
      setRefunds((current) =>
        current.map((item) =>
          item.id === refund.id && item.refundKind === refund.refundKind
            ? {
                ...item,
      ...
      setRefundMessage(
        "환불 계좌를 안전하게 등록했습니다. 운영자의 실제 송금 확인을 기다려 주세요.",
      );
    } catch (error) {
      setRefundMessage(
        error instanceof Error
  ...
  const openShippingRequest = () => {
    ...
    window.requestAnimationFrame(() => {
      document
        .getElementById("shipping-request")
  ...
  const homeActions: Array<HomeAction | null> = [
    pendingAuctionWins.length > 0
      ? {
          description: "결제 마감 전 입금을 진행해 주세요.",
          label: "낙찰품 결제",
          value: pendingAuctionWins.length,
          view: "payments",
          Icon: ReceiptText,
        }
      : null,
    visibleStorageItemCount > 0
      ? {
          description: "배송 신청 전 보관 상품을 확인해 주세요.",
          label: "보관 상품 확인",
          value: visibleStorageItemCount,
          view: "storage",
          Icon: PackageCheck,
        }
      : null,
    shipments.length > 0
      ? {
          description: "요청·발송 중인 배송 상태를 확인해 주세요.",
          label: "배송 현황 확인",
  ...
  const actionableHomeItems = homeActions.filter(
    (item): item is HomeAction => item !== null,
  );
  return (
    <div
      className={surface === "desktop" ? "space-y-14" : "space-y-10"}
      data-account-dashboard-view={view}
    >
      <div
        hidden={!showOverview}
        className={`flex justify-between gap-5 border-b border-ink pb-8 ${surface === "desktop" ? "flex-row items-end" : "flex-col"}`}
      >
        <div className="min-w-0">
          <p className="eyebrow text-muted">MY / 지금 할 일</p>
          <h1
            className={`mt-3 break-keep font-black tracking-[-0.08em] ${surface === "desktop" ? "text-4xl" : "text-3xl"}`}
          >
            안녕하세요, {userName}.
          </h1>
        ...
        {token ? (
          <div className="flex w-fit flex-wrap items-center gap-2">
            <button
              aria-label="로그아웃하기"
              className="inline-flex items-center gap-2 border border-line bg-paper px-4 py-3 text-xs font-bold disabled:opacity-40"
              disabled={logoutBusy}
              onClick={() => setLogoutConfirmOpen(true)}
              type="button"
            >
              <LogOut size={15} /> 로그아웃하기
            </button>
      ...
      {homeOnly && showOverview && !loading && token && (
        <section
          aria-labelledby="my-home-actions-title"
          className="border border-line bg-paper p-5 sm:p-7"
        >
          <div className="border-b border-line pb-4">
            <p className="eyebrow text-muted">MY / 지금 할 일</p>
            <h2 className="mt-2 text-xl font-black" id="my-home-actions-title">
              지금 처리해야 할 항목
            </h2>
            <p className="mt-2 text-xs leading-5 text-muted">
              확인이 필요한 작업만 모아 보여드려요.
            </p>
          </div>
          <div className="mt-5 divide-y divide-line border-y border-line">
            {actionableHomeItems.map(
              ({ description, label, value, view: actionView, Icon }) => (
                <button
                  className="flex w-full items-center gap-4 py-4 text-left transition-colors hover:bg-surface"
                  key={label}
                  onClick={() => onNavigate?.(actionView)}
                  type="button"
                >
      ...
      <div
        hidden={!showOverview || homeOnly}
        className={`grid gap-px border border-line bg-line ${surface === "desktop" ? "grid-cols-4" : "grid-cols-2"}`}
      >
        {visibleCards.map(([label, value, description, href, Icon]) => {
          const cardView: AccountDashboardView =
            label === "낙찰품 결제"
              ? "payments"
              : label === "보관 중인 상품"
                ? "storage"
          ...
          const cardContent = (
            <>
              <Icon size={17} />
              <p
                className={`text-xs text-muted ${surface === "desktop" ? "mt-8" : "mt-6"}`}
              >
                {label}
              </p>
              <p className="mt-2 font-mono text-3xl font-bold">
                {memberAccessRequired ? "—" : value}
              </p>
          ...
          return onNavigate ? (
            <button
              ...
              key={label}
              onClick={() => onNavigate(cardView)}
              type="button"
            >
              {cardContent}
            </button>
          ) : (
            <Link className={cardClassName} href={href} key={label}>
              {cardContent}
            </Link>
          );
        })}
      ...
      <section hidden={!showPayments} id="auction-payments">
        ...
        {pendingAuctionWins.length === 0 ? (
          ...
          <div className="border border-line bg-paper p-4 sm:p-5">
            <div
              className={`grid gap-3 ${surface === "desktop" ? "grid-cols-2" : "grid-cols-1"}`}
            >
      ...
      >
        <section className="contents">
          ...
          >
            <div
              className={`mb-5 flex items-start gap-3 border-b border-ink pb-4 ${surface === "desktop" ? "flex-row items-end justify-between" : "flex-col"}`}
            >
              <div>
              ...
              <div className="flex flex-wrap justify-end gap-2">
                ...
                <Link
                  className="border border-line bg-paper px-3 py-2 text-xs font-bold hover:bg-surface"
                  href={`${basePath}/chat`}
                >
                  배송 상담
                </Link>
            ...
            <div className="divide-y divide-line border-y border-line">
                        ...
                        onChange={(event) => {
                          setSelectedInventoryItemIds(
                            event.target.checked
                              ? visibleRequestEligibleItems.map(
                                  (item) => item.id,
                                )
                              : [],
                          );
                  ...
                  <div className="grid grid-cols-2 gap-2 p-1 md:grid-cols-3 xl:grid-cols-6">
                    {visibleV2Storage.map((item) => {
                      const startedAt = item.storageStartedAt
                        ...
                        : Number.NaN;
                      const explicitExpiresAt = item.storageExpiresAt
                        ? Date.parse(item.storageExpiresAt)
                        : Number.NaN;
                      const calculatedExpiresAt = Number.isFinite(startedAt)
                        ? startedAt + item.storageDurationDays * 86_400_000
                        : Number.NaN;
                      ...
                      const daysRemaining =
                        expires && now > 0
                          ? Math.max(
                              0,
                              Math.ceil((expires.getTime() - now) / 86_400_000),
                            )
                          : null;
                      const progress =
                        daysRemaining === null
                          ...
                          : Math.max(
                              ...
                              Math.min(
                                100,
                                (daysRemaining / item.storageDurationDays) *
                                  100,
                              ),
                            );
                      ...
                      const disabled =
                        !item.requestEligible || Boolean(item.activeShipmentId);
                          ...
                          >
                            ...
                            >
                              ...
                              <div
                                ...
                                role="progressbar"
                                aria-valuemin={0}
                                aria-valuemax={item.storageDurationDays}
                                aria-valuenow={
                                  daysRemaining ?? item.storageDurationDays
                                }
                              >
                                <span
                                  className={`block h-full rounded-full transition-[width] duration-300 ${daysRemaining === null ? "bg-zinc-500" : storageWarning ? "bg-red-600" : "bg-emerald-600"}`}
                                  style={{ width: `${progress}%` }}
                                />
                              </div>
                              {daysRemaining === 0 ? (
                                <p className="mt-2 inline-flex rounded-full border border-red-500/30 bg-red-500/10 px-2.5 py-1 text-[10px] font-black text-red-600">
                                  오늘 보관 만료
                                </p>
                              ) : daysRemaining !== null &&
                                daysRemaining <= 3 ? (
                                <p className="mt-2 rounded-xl border border-red-500/30 bg-red-500/10 px-2.5 py-2 text-[10px] font-bold text-red-600">
                                  만료 임박 · 묶음 배송 신청 권장
                                </p>
                              ) : null}
                            </div>
              ...
              {legacyEligibleOrders.length > 0 && (
                <div>
                  <div className="bg-surface px-3 py-3">
                    ...
                    <p className="mt-1 text-[11px] text-muted">
                      전환 전 결제 완료 상품은 주문 단위로 전체 배송을 신청할 수
                      있습니다.
                    </p>
                  </div>
                  {visibleLegacyEligibleOrders.map((order) => (
                    <label
                      className="block cursor-pointer px-1 py-5"
                      key={order.sourceId}
                    >
                      ...
                      <span
                        className={`mt-4 grid gap-3 ${surface === "desktop" ? "grid-cols-2" : "grid-cols-1"}`}
                      >
                        {order.items.map((item) => (
                          <span
                            className="flex min-w-0 items-center gap-3 border border-line p-3"
              ...
              {settledLegacyAuctionWins.length > 0 && (
                <div className="bg-surface px-3 py-5">
                  ...
                  <p className="mt-1 text-[11px] text-muted">
                    결제 완료 낙찰품을 통합 보관 상품으로 전환하고 있습니다.
                    전환이 끝나면 위 선택 목록에서 배송 신청할 수 있습니다.
                  </p>
                  <div
                    className={`mt-4 grid gap-3 ${surface === "desktop" ? "grid-cols-2" : "grid-cols-1"}`}
                  >
                    {visibleSettledLegacyAuctionWins.map((win) => (
                      <div
                        className="flex min-w-0 items-center gap-3 border border-line bg-paper p-3"
                        key={win.product_id}
                      >
                        <CatalogImage
                          alt=""
          ...
          >
            <div className="border border-line bg-surface p-4">
              ...
              <p className="mt-2 text-[11px] leading-5 text-muted">
                전환 완료 상품은 부분 선택해서, 전환 전 상품은 기존 주문 전체
                단위로 신청합니다. 두 방식을 동시에 선택해서 신청할 수는
                없습니다.
              </p>
                    ...
                    return selected ? (
                      <>
                        <p className="font-black">
                          {selected.label} · {selected.recipient_name}
                        </p>
              ...
              <p className="mt-3 text-[11px] text-muted">
                배송지 변경·추가·삭제는 통합 작업공간의 설정 → 배송지에서
                관리합니다.
              </p>
      ...
      >
        ...
        <div className="pb-4">
          {refundMessage && (
            <p
              aria-live="polite"
              className="mb-4 border border-line bg-surface px-4 py-3 text-xs"
            >
              {refundMessage}
            </p>
          )}
          <div className="divide-y divide-line border-y border-line">
            ...
            {refunds.map((refund) => {
              ...
              const accountExpired = Boolean(
                refund.accountExpiresAt &&
                now &&
                Date.parse(refund.accountExpiresAt) <= now,
              );
              const needsAccount =
              ...
              return (
                <article
                  className={`grid gap-5 py-5 ${surface === "desktop" ? "grid-cols-[minmax(0,1fr)_minmax(280px,420px)]" : "grid-cols-1"}`}
                  key={subjectKey}
                >
                  <div>
                    ...
                    <p className="mt-3 text-xs leading-5 text-rose-700">
                      {refund.refundKind === "item"
                        ? refund.publicReason
                        : "배송 요청 상품이 모두 제외되어 결제한 배송비를 돌려드립니다."}
                    </p>
                    {refund.accountSubmitted && !accountExpired && (
                      <p className="mt-2 text-[11px] text-muted">
                        환불 계좌가 안전하게 등록되었습니다. 운영자가 계좌를
                        열람하면 감사 기록이 남습니다.
                      </p>
                    )}
                    {accountExpired && (
                      <p className="mt-2 text-[11px] font-bold text-amber-700">
                        보호를 위해 계좌 등록 기간이 만료되었습니다. 다시 입력해
                        주세요.
                      </p>
                    )}
                  </div>
                  {needsAccount && (
                    <div
                      className={`grid gap-2 border border-line bg-surface p-4 ${surface === "desktop" ? "grid-cols-2" : "grid-cols-1"}`}
                    >
                      <input
                        aria-label={`${title} 환불 은행`}
                      ...
                      />
                      <button
                        className={`bg-ink px-4 py-3 text-xs font-bold text-paper disabled:opacity-40 ${surface === "desktop" ? "col-span-2" : ""}`}
                        disabled={Boolean(refundBusyId)}
                        onClick={() => void submitRefundAccount(refund)}
                        type="button"
                      >
                        {refundBusyId === subjectKey
                          ? "암호화 저장 중"
                          : "환불 계좌 등록"}
                      </button>
                    </div>
        ...
        <div className="divide-y divide-line border-y border-line">
          ...
          {shipments.map((shipment) => (
            <article className="py-5" key={shipment.id}>
              <div
                className={`flex gap-3 ${surface === "desktop" ? "flex-row items-start justify-between" : "flex-col"}`}
              >
                <div>
                  <p className="text-sm font-bold">
                    {shipment.publicStatus === "shipped"
                      ? "발송 완료"
                      : "배송 신청"}
                    {" · "}상품 {shipment.activeItemCount}/{shipment.itemCount}
                    개
                  </p>
                  <p className="mt-2 text-[11px] text-muted">
                    {shipment.requestedAt
                      ? new Date(shipment.requestedAt).toLocaleString("ko-KR")
                      : "요청 시각 확인 중"}
                    {" · "}
                    {shipment.publicStatus === "shipped"
                      ? "상품 발송"
                ...
                {shipment.trackingNumber && shipment.courier && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {!shipment.purchaseConfirmedAt &&
                      shipment.deliveredAt &&
                      shipment.purchaseConfirmationDueAt && (
                        <button
                          className="bg-ink px-3 py-2 text-xs font-bold text-paper"
                          onClick={() =>
                            setPurchaseConfirmationShipment(shipment)
                          }
                          type="button"
                        >
                          구매 확정하기
                    ...
                    {shipment.purchaseConfirmedAt && (
                      <span className="px-3 py-2 text-xs font-bold text-emerald-700">
                        구매 확정 완료
                      </span>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 border-t border-line p-3 sm:grid-cols-4">
                  {shipment.items.map((item) => (
                    <Link
                      ...
                      href={`${basePath}/auction/${item.productId}`}
                      key={`${shipment.id}:${item.productId}`}
                    >
                      <CatalogImage
                        alt=""
      ...
      <section hidden={!showAddresses} id="addresses">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-sm font-black">
                    {address.label} · {address.recipient_name}
                    {address.is_default && (
                      <span className="ml-2 border border-line px-2 py-0.5 text-[9px]">
                        기본
                      </span>
                    )}
                  </p>
                ...
                <button
                  className="shrink-0 border border-line px-3 py-2 text-xs font-bold"
        ...
        <button
          className="mt-4 min-h-12 w-full bg-ink px-4 text-sm font-black text-paper"
          onClick={openAddressCreate}
          type="button"
        >
          새 배송지 추가
      ...
      <PremiumDialog
        labelledBy="purchase-confirmation-title"
        onClose={() =>
          !purchaseConfirmationBusy && setPurchaseConfirmationShipment(null)
        }
        open={Boolean(purchaseConfirmationShipment)}
      ...
      >
        <div className="p-6">
          ...
          <p className="mt-4 text-sm leading-6">
            상품을 확인하고 구매 확정을 눌러주세요. 확정하면 거래가 완료되고
            판매자 정산 절차가 시작되며 되돌릴 수 없습니다.
          </p>
          <div className="mt-6 grid grid-cols-2 gap-2">
            <button
              className="h-11 border border-ink text-xs font-bold"
              disabled={purchaseConfirmationBusy}
              onClick={() => setPurchaseConfirmationShipment(null)}
              type="button"
            >
              취소
            </button>
            <button
              className="h-11 bg-ink text-xs font-bold text-paper disabled:opacity-50"
              disabled={purchaseConfirmationBusy}
              onClick={async () => {
                ...
                try {
                  const response = await fetch("/api/account/shipments", {
                    ...
                    body: JSON.stringify({ shipmentId: targetId }),
                  });
                  const payload = (await response.json().catch(() => null)) as {
                    error?: string;
                  } | null;
                  ...
                  setPurchaseConfirmationBusy(false);
                }
              }}
              type="button"
            >
              {purchaseConfirmationBusy ? "확정 중..." : "확정하기"}
            </button>
          </div>
        </div>
      </PremiumDialog>
      <PremiumDialog
        ...
        labelledBy="logout-confirmation-title"
      ...
      >
        <div className="p-6">
          ...
          <p className="mt-4 text-sm leading-6 text-muted">
            이 기기의 로그인 세션을 종료합니다. 장바구니와 주문 내역은 계정에
            안전하게 보관됩니다.
          </p>
          <div className="mt-6 grid grid-cols-2 gap-2">
            <button
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b border-line p-5">
          <div>
            ...
            <h2 className="mt-2 text-xl font-black" id="hanjin-tracking-title">
              택배사 사이트로 이동
            </h2>
          </div>
          <button
            aria-label="택배 조회 창 닫기"
            className="p-2"
        ...
        <div className="p-5 text-sm">
          <p className="font-bold">
            {trackingShipment?.courier} · {trackingShipment?.trackingNumber}
          </p>
          <p className="mt-2 text-xs leading-5 text-muted">
