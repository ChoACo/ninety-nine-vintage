...
export async function LoginPrompt({
  ...
  return <LoginSessionBoundary returnTo={returnTo}>
    <section className={`mx-auto w-full max-w-lg rounded-3xl border border-white/10 bg-paper text-center shadow-xl shadow-black/5 ${surface === "desktop" ? "p-10" : "p-6"}`}>
      <div className="mx-auto grid size-14 place-items-center rounded-2xl bg-kakao text-ink shadow-lg shadow-black/10"><MessageCircleMore size={25} /></div>
      ...
      <a className="mt-8 flex h-13 w-full items-center justify-center rounded-2xl bg-kakao text-sm font-bold text-kakao-foreground shadow-lg shadow-black/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl active:scale-95" href={loginHref}>카카오 로그인</a>
