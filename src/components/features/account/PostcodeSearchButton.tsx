...
export function PostcodeSearchButton({ disabled = false, onSelect }: { disabled?: boolean; onSelect: (result: { postalCode: string; address: string }) => void }) {
  ...
  return <>
    <Script onLoad={() => setReady(true)} src="https://t1.kakaocdn.net/mapjsapi/bundle/postcode/prod/postcode.v2.js" strategy="afterInteractive" />
    <button className="inline-flex min-h-11 items-center justify-center gap-2 border border-ink px-3 text-xs font-bold disabled:opacity-40" disabled={disabled} onClick={() => setOpen(true)} type="button"><Search size={14} />주소 검색</button>
    {open ? <div aria-labelledby="postcode-dialog-title" aria-modal="true" className="fixed inset-0 z-[100] grid place-items-center bg-black/65 p-4" role="dialog">
      <div className="flex max-h-[80vh] min-h-[520px] w-full max-w-md flex-col overflow-hidden rounded-2xl border border-line bg-paper shadow-2xl">
        ...
        {!ready ? <p className="p-5 text-xs text-muted" role="status">주소 검색 서비스를 불러오는 중입니다.</p> : <div className="min-h-0 flex-1" ref={hostRef} />}
      </div>
    </div> : null}
  </>;
}
