    ...
    <article className="group min-w-0">
      <Link className="block" href={`/auction/${item.id}`}>
        <div className="relative aspect-[4/5] overflow-hidden bg-surface">
          ...
          <span className="absolute left-2 top-2 bg-paper px-2 py-1 font-mono text-[9px] font-bold tracking-[0.1em]">{isFixed ? "즉시 구매" : "실시간 입찰"}</span>
          <button aria-label={liked ? "찜 해제" : "찜하기"} className={`absolute right-2 top-2 grid size-8 place-items-center bg-paper/90 transition-colors ${liked ? "text-red-700" : "text-ink"}`} onClick={(event) => { event.preventDefault(); void updateWishlist(); }} type="button"><Heart fill={liked ? "currentColor" : "none"} size={15} strokeWidth={1.6} /></button>
      ...
      <div className="pt-3">
        ...
        <div className="mt-4 grid grid-cols-2 gap-2">
          {isFixed ? <><button className="flex h-9 items-center justify-center gap-1 border border-line text-[10px] font-bold transition-colors hover:border-ink disabled:opacity-50" disabled={cartBusy} onClick={(event) => { event.preventDefault(); void addFixedToCart(); }} type="button"><ShoppingBag size={13} /> {cartBusy ? "저장 중" : "장바구니"}</button><Link className="flex h-9 items-center justify-center bg-ink text-[10px] font-bold text-paper" href={`/auction/${item.id}`}>즉시 구매</Link></> : <><Link className="flex h-9 items-center justify-center gap-1 bg-ink text-[10px] font-bold text-paper" href={`/auction/${item.id}/bid`}><Gavel size={13} /> 입찰하기</Link><button className="flex h-9 cursor-not-allowed items-center justify-center gap-1 border border-line text-[10px] font-bold text-muted" disabled title="경매 상품은 장바구니에 담을 수 없습니다." type="button"><ShoppingBag size={13} /> 장바구니</button></>}
        </div>
