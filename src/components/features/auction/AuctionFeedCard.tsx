  ...
  return (
    <article className="group min-w-0 border-b border-line pb-5" data-auction-phase={phase} data-bid-capability={bidCapability} data-participation-state={participationState ?? "none"}>
      ...
      <div className="pt-3">
        ...
        <div className="mt-1 flex items-start justify-between gap-2"><Link className="min-w-0 truncate text-sm font-medium hover:underline" href={`/auction/${item.id}`}>{item.name}</Link>{participationState && <span className={`shrink-0 rounded-lg border px-2 py-1 text-[9px] font-bold ${participationState === "leading" || participationState === "final" ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-50 text-amber-900"}`}>{participationLabels[participationState]}</span>}</div>
      ...
      <AuctionGalleryModal images={galleryImages} key={item.id} onClose={() => setGalleryOpen(false)} open={galleryOpen} title={item.name} />
    </article>
  );
}
