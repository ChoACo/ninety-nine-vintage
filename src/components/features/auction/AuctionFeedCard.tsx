...
function EnabledAuctionFeedCard({ bidCapability, item, participationState }: AuctionFeedCardProps) {
    ...
    <article className="group min-w-0 border-b border-line pb-5" data-auction-phase={phase} data-bid-capability={bidCapability} data-participation-state={participationState ?? "none"}>
      <div className="relative aspect-[4/5] overflow-hidden bg-surface">
        <Link className="block h-full" href={`/auction/${item.id}`}>
          ...
          <span className="absolute left-2 top-2 bg-paper px-2 py-1 font-mono text-[9px] font-bold tracking-[0.1em]">실시간 입찰</span>
        ...
        <button aria-label={liked ? "찜 해제" : "찜하기"} className={`absolute right-2 top-2 grid size-8 place-items-center bg-paper/90 transition-colors ${liked ? "text-red-700" : "text-ink"}`} onClick={() => void updateWishlist()} type="button"><Heart fill={liked ? "currentColor" : "none"} size={15} strokeWidth={1.6} /></button>
        {galleryImages.length > 0 && <button aria-label={`${item.name} 사진 ${galleryImages.length}장 확대 보기`} className="absolute bottom-2 right-2 flex h-8 items-center gap-1 bg-paper/90 px-2 text-[9px] font-bold text-ink" onClick={() => setGalleryOpen(true)} type="button"><Images size={13} /> {galleryImages.length}</button>}
      </div>
      ...
      <div className="pt-3">
        ...
        <div className="mt-1 flex items-start justify-between gap-2"><Link className="min-w-0 truncate text-sm font-medium hover:underline" href={`/auction/${item.id}`}>{item.name}</Link>{participationState && <span className={`shrink-0 border px-2 py-1 text-[9px] font-bold ${participationState === "leading" || participationState === "final" ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-50 text-amber-900"}`}>{participationLabels[participationState]}</span>}</div>
        ...
        <div className="mt-4 grid grid-cols-[1fr_auto_auto] gap-2">{canStartBid ? <Link className="flex h-9 items-center justify-center gap-1 bg-ink text-[10px] font-bold text-paper" href={`/auction/${item.id}/bid`}><Gavel size={13} /> {bidLabel}</Link> : <button className="flex h-9 items-center justify-center gap-1 bg-zinc-300 text-[10px] font-bold text-paper" disabled type="button"><Gavel size={13} /> {bidLabel}</button>}<button aria-label={`${item.name} 상품 문의`} className="grid size-9 place-items-center border border-line" onClick={() => setInquiryOpen(true)} type="button"><MessageCircle size={13} /></button><Link className="flex h-9 items-center justify-center border border-line px-3 text-[10px] font-bold" href={`/auction/${item.id}`}>상세</Link></div>
      ...
      <ProductInquiryModal onClose={() => setInquiryOpen(false)} open={inquiryOpen} productId={item.id} productTitle={item.name} />
      <AuctionGalleryModal images={galleryImages} key={`${galleryOpen}-${item.id}`} onClose={() => setGalleryOpen(false)} open={galleryOpen} title={item.name} />
    </article>
  );
}
