...
function EnabledAuctionFeedCard({ basePath = "", bidCapability, item, participationState, surface = basePath === "/m" ? "mobile" : "desktop" }: AuctionFeedCardProps) {
  ...
  return (
    <article className="product-card group min-w-0 border-b border-line pb-5" data-auction-phase={phase} data-bid-capability={bidCapability} data-participation-state={participationState ?? "none"}>
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl border border-line/30 bg-muted/20 shadow-sm transition-all duration-300 group-hover:border-line/80 group-hover:-translate-y-1 group-hover:shadow-xl">
        <Link className="block h-full" href={`${basePath}/auction/${item.id}`}>
          {item.imageUrl ? <CatalogImage alt={`${item.brand} ${item.name}`} className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" fill loading="lazy" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" src={item.imageUrl} /> : <div className="grid h-full place-items-center text-xs text-muted">이미지 준비 중</div>}
          <div className="absolute left-2.5 top-2.5 flex flex-col items-start gap-1.5">
            {grade && <span className="rounded-full bg-zinc-900/90 px-2 py-1 font-mono text-[9px] font-black tracking-[0.08em] text-white shadow-sm backdrop-blur-md">GRADE {grade}</span>}
            <span aria-label={phase === "CLOSED" ? "마감됨" : "실시간 입찰"} className={`rounded-full px-2 py-1 text-[9px] font-black tracking-[0.12em] text-white shadow-sm ${phase === "CLOSED" ? "bg-zinc-700" : "bg-rose-600"}`}>{phase === "CLOSED" ? "마감됨" : "LIVE"}</span>
          ...
          {quickSpecs.length > 0 && <div aria-hidden="true" className="product-quick-specs pointer-events-none absolute inset-x-3 bottom-3 z-10 translate-y-2 rounded-xl border border-white/15 bg-ink/90 px-3 py-2.5 text-paper opacity-0 shadow-xl backdrop-blur-md transition-[opacity,transform] duration-200"><p className="line-clamp-2 text-[10px] font-bold leading-4">{quickSpecs.join(" · ")}</p></div>}
        </Link>
      ...
      <div className="pt-3">
        ...
        <div className="mt-1 flex items-start justify-between gap-2"><Link className="min-h-[1.5rem] min-w-0 line-clamp-1 break-keep text-sm font-medium text-foreground/90 hover:underline" href={`${basePath}/auction/${item.id}`}>{item.enhancedTitle || item.name}</Link>{participationState && <span className={`shrink-0 rounded-lg border px-2 py-1 text-[9px] font-bold ${participationState === "leading" || participationState === "final" ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-amber-500/10 text-amber-900"}`}>{participationLabels[participationState]}</span>}</div>
        ...
        <div className="mt-3 border-y border-line py-3">
          <div className="flex items-end justify-between gap-2"><div><p className="text-[10px] text-muted">{phase === "CLOSED" ? "최종 낙찰가" : "현재 최고 입찰가"}</p><p className="mt-1 font-mono text-base font-bold text-foreground tabular-nums">{currentPrice.toLocaleString("ko-KR")}원</p></div><button aria-label={`입찰 내역 ${bidCount}건 보기`} className="flex items-center gap-1 text-right text-[10px] text-muted underline" onClick={() => setHistoryOpen(true)} type="button"><List size={12} /> 입찰 {bidCount}건 · 참여 {participantCount}명</button></div>
        </div>
