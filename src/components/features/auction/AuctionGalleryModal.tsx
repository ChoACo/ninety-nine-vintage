    ...
    >
      <header className={`flex shrink-0 items-center justify-between gap-3 border-b border-white/10 bg-zinc-950/90 backdrop-blur-md ${surface === "desktop" ? "h-18 px-6" : "min-h-16 pb-3 pl-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] pt-[max(0.75rem,env(safe-area-inset-top))]"}`}>
        ...
        <div className="flex items-center gap-1.5 rounded-2xl border border-white/10 bg-white/5 p-1.5 shadow-xl shadow-black/20">
          <button aria-label="사진 축소" className="grid size-11 place-items-center rounded-xl transition-all duration-300 hover:-translate-y-0.5 hover:bg-card hover:text-zinc-950 active:scale-95 disabled:opacity-35" disabled={transform.scale <= 1} onClick={() => zoomTo(transform.scale / 1.35)} type="button"><Minus size={18} /></button>
          ...
          <button aria-label="사진 확대" className="grid size-11 place-items-center rounded-xl transition-all duration-300 hover:-translate-y-0.5 hover:bg-card hover:text-zinc-950 active:scale-95" onClick={() => zoomTo(transform.scale * 1.35)} type="button"><Plus size={18} /></button>
          <span aria-hidden="true" className="mx-1 h-7 w-px bg-white/15" />
          <button aria-label="사진 확대 보기 닫기" className="grid size-11 place-items-center rounded-xl transition-all duration-300 hover:-translate-y-0.5 hover:bg-card hover:text-zinc-950 active:scale-95" onClick={onClose} ref={closeButtonRef} type="button"><X size={19} /></button>
        </div>
      </header>
