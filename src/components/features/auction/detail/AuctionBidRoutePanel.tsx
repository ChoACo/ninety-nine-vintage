...
export function AuctionBidRoutePanel({ basePath = "", bidIncrement, currentPrice, minimumBid, productId, productTitle }: AuctionBidRoutePanelProps) {
  ...
  return (
    ...
    <section className={`mx-auto w-full max-w-xl rounded-3xl border border-white/10 bg-paper shadow-xl shadow-black/5 ${surface === "desktop" ? "p-3" : "p-1"}`}>
      ...
      {!loading && !session ? <Link className="mt-5 flex h-13 w-full items-center justify-center rounded-2xl bg-kakao text-sm font-bold text-kakao-foreground shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl active:scale-95" href={`${basePath}/account/login?next=${encodeURIComponent(`${basePath}/auction/${productId}/bid`)}`}>카카오 로그인 후 입찰</Link> : <button className="mt-5 flex h-13 w-full items-center justify-center gap-2 rounded-2xl bg-ink text-sm font-bold text-paper shadow-lg shadow-black/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl active:scale-95 disabled:opacity-40" disabled={busy || loading || !session || isCoolingDown} onClick={requestConfirmation} type="button">{busy ? <><LockKeyhole size={16} /> 서버에 저장 중</> : isCoolingDown ? <><LockKeyhole size={16} /> {cooldownSeconds}초 후 다시 시도</> : <><Gavel size={16} /> 입찰 내용 확인</>}</button>}
    </section>
    <PremiumDialog closeDisabled={busy} labelledBy="bid-final-confirm-title" onClose={() => setConfirmOpen(false)} open={confirmOpen} panelClassName="max-w-lg">
      ...
      <div className="p-6">
        ...
        <p className="mt-4 rounded-2xl border border-amber-200 bg-amber-500/10 px-4 py-3 text-xs font-bold leading-relaxed text-amber-900">입찰은 제출 후 취소할 수 없으며, 낙찰 시 안내된 기한 안에 결제해야 합니다.</p>
        <div className="mt-6 grid grid-cols-2 gap-2"><Button disabled={busy || isCoolingDown} onClick={() => setConfirmOpen(false)} type="button">금액 다시 보기</Button><Button disabled={busy || isCoolingDown} onClick={() => void submit()} type="button" variant="primary">{busy ? "서버에 저장 중" : isCoolingDown ? `${cooldownSeconds}초 후 다시 시도` : "동의하고 최종 입찰"}</Button></div>
      </div>
    </PremiumDialog>
    </>
  );
