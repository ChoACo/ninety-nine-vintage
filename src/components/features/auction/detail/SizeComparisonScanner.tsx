...
function ProfileEditor({
  ...
  return (
    <form className="space-y-4" onSubmit={submit}>
      ...
      {error && <p className="border border-red-200 bg-red-50 px-3 py-2 text-xs font-bold text-red-700" role="alert">{error}</p>}
...
export function SizeComparisonScanner({
  ...
  return (
    <div className="fixed inset-0 z-[115] flex items-center justify-center bg-zinc-950/70 px-6 py-8" onMouseDown={(event) => event.target === event.currentTarget && onClose()} role="presentation">
      <section aria-labelledby="size-comparison-title" aria-modal="true" className="max-h-[86vh] w-[min(720px,calc(100vw-3rem))] overflow-y-auto border border-line bg-paper text-ink shadow-2xl" role="dialog">
        <header className="flex items-start justify-between gap-6 border-b border-line px-6 py-5">
          <div className="min-w-0">
            <p className="flex items-center gap-2 text-[10px] font-bold tracking-[0.14em] text-muted"><Ruler size={13} /> 옷과 옷 실측 비교</p>
            <h2 className="mt-2 text-xl font-black tracking-[-0.04em]" id="size-comparison-title">내 옷과 실측 비교</h2>
            <p className="mt-2 truncate text-xs text-muted">{productTitle}</p>
          </div>
          <button aria-label="실측 비교 닫기" className="shrink-0 text-muted hover:text-ink" onClick={onClose} type="button"><X size={19} /></button>
        </header>

        <div className="p-6">
          <div className="mb-5 flex items-center justify-between border border-line bg-surface px-4 py-3">
            ...
            <span className="border border-line bg-paper px-2 py-1 font-mono text-[10px] font-bold text-muted">상품 실측 {productMeasurementCount}개</span>
          </div>
          ...
          {!garment.hydrated ? (
            ...
            <div className="border border-dashed border-line bg-surface px-5 py-10 text-center"><p className="text-sm font-bold">상품 실측 정보 준비 중</p><p className="mt-2 text-xs leading-5 text-muted">실측값이 두 개 이상 등록되면 자동으로 비교합니다.</p></div>
          ) : (
            <div className="space-y-3">
              {comparisons.map((comparison) => (
                <div className="border border-line px-4 py-4" key={comparison.key}>
