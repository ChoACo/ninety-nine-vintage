...
export function ProductRail({ products, title, eyebrow, href = "/feed", compact = false }: { products: PublishedProduct[]; title: string; eyebrow: string; href?: string; compact?: boolean }) {
  return <section className="mt-20"><div className="mb-6 flex items-end justify-between border-b border-ink pb-4"><div><p className="eyebrow text-muted">{eyebrow}</p><h2 className="mt-2 text-2xl font-black tracking-[-0.06em]">{title}</h2></div><Link className="flex items-center gap-1 text-xs font-bold hover:underline" href={href}>전체 보기 <ArrowUpRight size={14} /></Link></div>{compact ? <div className="grid gap-2 md:grid-cols-2 lg:grid-cols-3">{products.map((product) => <Link className="flex min-w-0 items-center gap-3 border-b border-line py-3 transition-colors hover:bg-surface" href={"/auction/" + product.id} key={product.id}><img alt="" className="size-14 shrink-0 object-cover" loading="lazy" src={getCatalogImageUrl(product.thumbnailUrls[0] ?? product.imageUrls[0] ?? "", 320)} /><span className="min-w-0 flex-1"><span className="block truncate text-xs font-bold">{product.title}</span><span className="mt-1 block text-[10px] text-muted">{product.sizeLabel || "사이즈 미등록"} · {product.saleType === "fixed" ? "즉시구매" : "경매"}</span></span><span className="shrink-0 text-right"><span className="block font-mono text-xs font-bold">{(product.saleType === "fixed" ? product.fixedPrice ?? product.currentPrice : product.currentPrice).toLocaleString("ko-KR")}원</span><span className="mt-1 block text-[10px] text-muted">{product.saleType === "fixed" ? "BUY NOW" : "LIVE"}</span></span></Link>)}</div> : <div className="grid grid-cols-2 gap-x-3 gap-y-9 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{products.map((product) => <AuctionCard item={toItem(product)} key={product.id} />)}</div>}{products.length === 0 && <div className="border border-dashed border-line py-16 text-center text-sm text-muted">현재 공개된 상품이 없습니다.</div>}</section>;
}
