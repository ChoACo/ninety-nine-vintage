...
export function StoreMallExperience({ auctions, basePath = "", dates, fixed, routeSegment = "stores", selectedDate, slug, sold, store, surface }: StoreMallExperienceProps) {
  ...
  return <div className={isDesktop ? "pb-24" : "pb-8"}>
    ...
    {secondary.length > 0 && <section className={`mt-8 grid gap-3 ${isDesktop ? "grid-cols-2" : "grid-cols-1"}`} aria-label="센터 추천 상품">{secondary.map((product) => <Link className="group grid grid-cols-[120px_1fr] overflow-hidden border border-line bg-paper" href={`${basePath}/${product.saleType === "fixed" ? "shop" : "auction"}/${product.id}`} key={product.id}><CatalogImage alt={`${product.brand} ${product.title}`} className="aspect-square size-full object-cover transition-transform group-hover:scale-105" maxDimension={480} sizes="120px" src={product.thumbnailUrls[0] ?? product.imageUrls[0] ?? ""} /><div className="flex min-w-0 flex-col justify-between p-4"><div><p className="text-[9px] font-bold tracking-[.16em] text-muted">STORE PICK</p><p className="mt-2 truncate text-sm font-black">{product.title}</p></div><p className="font-mono text-xs font-bold">{priceOf(product).toLocaleString("ko-KR")}원</p></div></Link>)}</section>}
