...
export function CommerceToolbar() {
  ...
  return <div className="flex shrink-0 items-center gap-2"><Link aria-busy={visibleLikedCount === null} aria-label="찜한 상품" className="relative grid size-10 shrink-0 place-items-center border border-line" href="/account#likes"><Heart size={16} />{visibleLikedCount !== null && <span className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-ink text-[9px] text-paper">{visibleLikedCount}</span>}</Link>{auctionContext ? <Link aria-label="입찰 현황" className="relative grid size-10 shrink-0 place-items-center border border-line" href="/account#bids"><Gavel size={16} /><span className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-ink px-1 text-[8px] text-paper">BID</span></Link> : <Link aria-busy={visibleCartCount === null} aria-label="장바구니" className="relative grid size-10 shrink-0 place-items-center border border-line" href="/cart"><ShoppingBag size={16} />{visibleCartCount !== null && <span className="absolute -right-1 -top-1 grid size-4 place-items-center rounded-full bg-ink text-[9px] text-paper">{visibleCartCount}</span>}</Link>}</div>;
}
