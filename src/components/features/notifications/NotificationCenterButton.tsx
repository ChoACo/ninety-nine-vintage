...
export function NotificationCenterButton() {
  ...
  useEffect(() => {
    ...
    void fetch("/api/notifications", { headers: { Authorization: `Bearer ${token}` }, cache: "no-store" })
      .then((response) => response.ok ? response.json() as Promise<{ notifications?: NotificationItem[]; viewerRole?: NotificationViewerRole }> : Promise.reject(new Error("notifications_unavailable")))
      .then((payload) => {
        if (!active) return;
  ...
  const markRead = async (item: NotificationItem) => {
    ...
    if (!item.read_at) {
      ...
      await fetch("/api/notifications", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ notificationId: item.id }) }).catch(() => undefined);
    }
    setOpen(false);
  ...
  const markAllRead = async () => {
    ...
    await fetch("/api/notifications", { method: "POST", headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" }, body: JSON.stringify({ markAll: true }) }).catch(() => undefined);
  };
