...
export function OperatorChatConsole() {
  ...
  const loadInbox = async (accessToken: string) => {
    const response = await fetch("/api/operator/chat", { headers: { Authorization: `Bearer ${accessToken}` }, cache: "no-store" });
