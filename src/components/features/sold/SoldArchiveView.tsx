...
export async function SoldArchiveView({ brandSlug, before, beforeId }: { brandSlug?: string; before?: string; beforeId?: string }) {
  ...
  return <div className="space-y-8 md:space-y-10">
    <div className="border-b border-ink pb-7"><p className="eyebrow text-muted">판매 기록 / 판매 완료</p><h1 className="mt-3 break-keep text-3xl font-black tracking-[-.08em] md:text-4xl">{activeBrand ? `${activeBrand.brand} 판매 기록` : "판매 완료 아카이브"}</h1><p className="mt-3 text-sm text-muted">다시 만날 수 없기에, 기록으로 남깁니다.</p></div>
