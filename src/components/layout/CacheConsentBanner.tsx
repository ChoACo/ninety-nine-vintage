...
export function CacheConsentBanner() {
  ...
  return <aside className="fixed inset-x-3 bottom-20 z-50 mx-auto flex max-w-xl items-start gap-3 border border-line bg-paper p-4 text-ink shadow-lg lg:inset-x-auto lg:right-6 lg:bottom-6"><Database className="mt-0.5 shrink-0" size={18} /><div className="min-w-0 flex-1"><p className="text-xs font-bold">빠른 로딩을 위한 공개 캐시</p><p className="mt-1 text-[11px] leading-5 text-muted">공개 상품·이미지·정적 리소스만 기기에 저장합니다. 계정·주문·결제 정보는 저장하지 않습니다.</p><div className="mt-3 flex gap-2"><button className="inline-flex items-center gap-1 bg-ink px-3 py-2 text-[11px] font-bold text-paper" onClick={accept} type="button"><Check size={13} /> 허용</button><button className="inline-flex items-center gap-1 border border-line px-3 py-2 text-[11px] font-bold" onClick={decline} type="button"><X size={13} /> 거부</button></div></div></aside>;
}
