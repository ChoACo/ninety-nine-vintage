...
export function LiveTickerBar() {
  ...
  return (
    <aside className="sticky top-0 z-50 h-9 border-b border-zinc-800 bg-zinc-950 text-white">
      <div className="mx-auto flex h-full max-w-[1680px] items-center justify-between gap-3 px-4 text-[10px] sm:px-6 md:px-8 lg:px-10 xl:px-12 sm:text-xs">
        <div className="flex min-w-0 items-center gap-2 font-medium">
          <span className="flex items-center gap-1.5 tracking-[0.12em] text-emerald-400">
            <CircleDot size={14} strokeWidth={2.5} />
            LIVE DROP
          </span>
          <span className="truncate text-zinc-300">오늘의 빈티지 드롭 · 21:00 KST 마감</span>
        </div>
        <div className="flex items-center gap-4">
          <strong className="shrink-0 font-mono text-xs tracking-[0.08em] sm:text-sm">
            {timeLeft} 남음
          </strong>
          <span className="hidden text-zinc-400 md:inline">20:56 신규 참여 제한</span>
        </div>
      </div>
    </aside>
  );
