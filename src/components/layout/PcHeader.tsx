...
const navigation = [
  ...
  ...(LIVE_AUCTION_ENABLED ? [{ label: "LIVE AUCTION", href: "/feed" }] : []),
...
export function PcHeader({ hasLiveTicker = false }: { hasLiveTicker?: boolean }) {
  ...
  return (
    <header className={`sticky ${hasLiveTicker ? "top-9" : "top-0"} z-[60] border-b border-line bg-paper/95 text-ink`}>
      <div className="mx-auto flex h-20 max-w-[1680px] items-center justify-between gap-4 px-10 xl:px-12">
        <button aria-label="메뉴 열기" className="grid size-10 place-items-center border border-line hidden" onClick={() => setOpen(true)} type="button"><Menu size={18} /></button>
        <Link className="min-w-0 w-[270px] shrink text-lg font-black tracking-[-0.06em]" href="/home">NINETY-NINE VINTAGE</Link>
        ...
        <div className="flex shrink-0 items-center gap-2 ml-4 justify-end">
