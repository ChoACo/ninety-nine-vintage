...
import { Clock, Gavel, Headphones, Home, Search, Store } from "lucide-react";
...
export function PcHeader({ hasLiveTicker = false }: { hasLiveTicker?: boolean }) {
  ...
  const standardNavigation: ReadonlyArray<{ label: string; href: string; icon: LucideIcon }> = [
    ...
    { label: "센터몰", href: "/stores", icon: Store },
    ...(LIVE_AUCTION_ENABLED && hasActiveBid ? [{ label: "입찰 중인 상품", href: "/bidding", icon: Clock }] : []),
    ...(LIVE_AUCTION_ENABLED ? [{ label: "실시간 경매", href: "/feed", icon: Gavel }] : []),
    { label: "즉시 구매", href: "/shop", icon: Store },
  ];
  ...
  return (
    <header className={`sticky ${hasLiveTicker ? "top-9" : "top-0"} z-[60] block border-b border-line bg-paper/95 text-ink backdrop-blur-md`}>
      <div className="mx-auto flex h-20 w-full max-w-[1440px] items-center gap-5 px-5 sm:px-8 xl:px-10">
        ...
        <ThemeToggle className="ml-4 shrink-0 size-10 px-0" />
        ...
        <div className="ml-auto flex shrink-0 items-center justify-end gap-2">
          {!simpleMode.enabled && <nav aria-label="주요 메뉴" className="mr-4 flex shrink-0 items-center gap-1.5">
            {standardNavigation.map((item) => {
              const Icon = item.icon;
              const active = pathname === item.href;
              return <Link aria-current={active ? "page" : undefined} className={`inline-flex h-10 shrink-0 items-center gap-1.5 whitespace-nowrap border px-3 text-[11px] font-bold transition-colors ${active ? "border-ink bg-surface text-ink" : "border-line text-muted hover:border-ink hover:text-ink"}`} href={item.href} key={item.href}>{<Icon size={15} />}{item.label}</Link>;
            })}
          </nav>}
          {!simpleMode.enabled && <>
