-- Make combined-shipping requests safe to retry from multiple tabs or after
-- a lost network response. Existing requests remain valid with a null key.

alter table public.shipping_requests
  add column if not exists idempotency_key text;

create unique index if not exists shipping_requests_member_idempotency_idx
  on public.shipping_requests (member_id, idempotency_key)
  where idempotency_key is not null;

alter table public.shipping_fee_payments
  add column if not exists idempotency_key text;

create unique index if not exists shipping_fee_payments_member_idempotency_idx
  on public.shipping_fee_payments (member_id, idempotency_key)
  where idempotency_key is not null;
