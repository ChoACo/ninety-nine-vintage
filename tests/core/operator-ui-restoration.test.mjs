...
test("operator product console publishes directly and manages active listings from explicit store permission", async () => {
  ...
  assert.match(products, /<MeasurementFields[\s\S]{0,120}category=\{form\.category\}/);
  ...
  assert.match(products, />[\s\S]{0,80}점검\s*<\/button>/);
  ...
  assert.match(products, /form\.status === "active" \|\|\s*stores\.find/);
  assert.match(products, /disabled=\{\s*busy \|\|\s*!canPublishStore \|\|\s*product\.status !== "pending"\s*\}/);
  assert.match(products, /\/products\/\$\{product\.id\}\/pause/);
  assert.match(products, />[\s\S]{0,80}일시중지\s*<\/button>/);
  ...
  assert.match(productRoute, /auth\.user[\s\S]{0,40}\.from\("products"\)[\s\S]{0,40}\.insert/);
  ...
  assert.match(productRoute, /user[\s\S]{0,40}\.from\("products"\)[\s\S]*\.in\("store_id", storeIds\)/);
  ...
  assert.match(patchRoute, /product\.status === "active" &&\s*saleSetupFields\.some/);
