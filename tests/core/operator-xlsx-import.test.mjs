...
test("operator XLSX UI validates products, supports per-product deletion, and requires explicit confirmation", async () => {
  ...
  assert.match(modal, /selectedFiles\.filter\(isWorkbookFile\)/);
  assert.match(modal, /isSupportedProductImageMimeType\(file\.type\)/);
  assert.match(modal, /workbookFiles\.length !== 1/);
  assert.match(modal, /등록용 \.xlsx 엑셀 파일이 1개만 있어야 합니다/);
  assert.match(modal, /엑셀·상품 이미지 폴더/);
  assert.match(modal, /폴더를 한 번만 선택하세요/);
