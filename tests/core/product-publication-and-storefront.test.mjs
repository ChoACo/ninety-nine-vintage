...
test("store malls split into main, new, auction, buy, and info pages", async () => {
  ...
  assert.match(storeTabs, /{ key: "main", label: "메인"/);
  assert.match(storeTabs, /{ key: "new", label: "신상품"/);
  assert.match(storeTabs, /{ key: "auction", label: "경매"/);
  assert.match(storeTabs, /{ key: "buy", label: "구매"/);
  assert.match(storeTabs, /{ key: "about", label: "정보"/);
