...
test("browser auth rejects URL-provided Supabase sessions", async () => {
  ...
  assert.match(sessionHook, /sessionDiscardFlight\?\.accessToken === accessToken/);
  assert.match(sessionHook, /current\?\.access_token === rejected\.access_token/);
  assert.match(sessionHook, /invalid_grant/);
  assert.match(sessionHook, /refresh_token_not_found/);
  assert.match(sessionHook, /refresh_token_already_used/);
  assert.match(sessionHook, /discardInvalidSession\(client, null\)/);
  assert.match(sessionHook, /clearSupabaseBrowserSessionStorage\(\)/);
